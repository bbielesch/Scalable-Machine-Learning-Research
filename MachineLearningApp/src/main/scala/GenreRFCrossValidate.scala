...
object GenreRFCrossValidateApp {
  def main(args: Array[String]) {
    ...
    // extract the most relevant information, features, labels and splits to apply
    val featuresFile = configuration.getString("ml.files.featuresFile") 
    ...
    val numTrees: Array[Int] = configuration
      .getIntList("ml.params.numTrees")
      .asScala.toArray.map(_.toInt)
    println(dateFormatter.format(new java.util.Date()) 
        + " CONFIG Random Forest: Number of trees: " 
        + numTrees.mkString(" "))
    
    val maxDepth: Array[Int] =  configuration
      .getIntList("ml.params.maxDepth")
      .asScala.toArray.map(_.toInt)
    println(dateFormatter.format(new java.util.Date()) 
        + " CONFIG Random Forest: Maximum tree depth: " 
        + maxDepth.mkString(" "))
    
    val maxBins: Array[Int] = configuration
      .getIntList("ml.params.maxBins")
      .asScala.toArray.map(_.toInt)
    println(dateFormatter.format(new java.util.Date()) 
        + " CONFIG Random Forest: Maximum number of bins: " 
        + maxBins.mkString(" "))

    val featureSubsetStrategy: Array[String] = configuration
      .getStringList("ml.params.featureSubsetStrategy")
      .asScala.toArray
    println(dateFormatter.format(new java.util.Date()) 
        + " CONFIG Random Forest: Feature subset strategy: " 
        + featureSubsetStrategy.mkString(" "))  
    
    val impurity: Array[String] = configuration
      .getStringList("ml.params.impurity")
      .asScala.toArray
    println(dateFormatter.format(new java.util.Date()) + 
        " CONFIG Random Forest: Impurity: " 
        + impurity.mkString(" "))
    ...
    //and now access Spark and start working
    ...
    */
    ...
    // train cross validator model. This also runs the indexers
    val crossValidatorModel = cv.fit(trainData)

    // output parameters for best model
    // println(dateFormatter.format(new java.util.Date()) + " Best Model: " 
