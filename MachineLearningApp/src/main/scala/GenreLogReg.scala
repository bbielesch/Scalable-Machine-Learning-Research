...
object GenreLogRegApp {
  def main(args: Array[String]) {
    ...
    println(dateFormatter.format(new java.util.Date()) + " CONFIG Logistics Regression: Max number of iterations: " + maxIter)
    
    val regParam =
       if (configuration.hasPath("ml.params.regParam")) 
         configuration.getDouble("ml.params.regParam") else 0.3
    println(dateFormatter.format(new java.util.Date()) + " CONFIG Logistics Regression: Regularization parameter: " + regParam)
    
    val elasticNetParam = 
      if (configuration.hasPath("ml.params.elasticNetParam")) 
        configuration.getDouble("ml.params.elasticNetParam") else 0.8
    println(dateFormatter.format(new java.util.Date()) + " CONFIG Logistics Regression: ElasticNet mixing parameter: " + elasticNetParam)  
    ...
    */
    ...
    //output coefficients and intercept of multinomial logistic regression
    val logRegModel = model
      .stages.apply(2) //index starts with 0 and logistic model is 3rd entry in list
      .asInstanceOf[LogisticRegressionModel]

    println(dateFormatter.format(new java.util.Date()) + " RESULT Coefficients: " + 
        logRegModel.coefficientMatrix)
    println(dateFormatter.format(new java.util.Date()) + " RESULT Intercepts: " + 
